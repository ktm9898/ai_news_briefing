...
3. 각 인사이트 항목은 'title'(뉴스 제목, 원문 그대로 생략 없이 전체 작성), 'summary'(주요내용), 'implication'(시사점)으로 구성하십시오.
   ...
   - **[외부 사례 출처 표기]**: 시사점(implication)에서 새롭게 제시한 외부 유사 사례나 레퍼런스가 있다면, 해당 출처(예: 언론사명, 보고서명 등)를 'reference' 필드에 분리하여 작성하십시오. 현재 요약 중인 원본 기사의 출처는 적지 마십시오. 외부 사례를 인용하지 않았다면 빈 문자열("")로 비워두십시오.
...
"""
    ...
    \"news_insights\": [
      {{
        ...
        \"reference\": \"참고한 외부 사례 출처 (없으면 빈 문자열)\"
      }}
    ]
  }}"""
