...
3. 각 인사이트 항목은 'title'(뉴스 제목), 'summary'(주요내용), 'implication'(인사이트), 'references'(외부 출처 배열)로 구성하십시오.
   ...
   - **[가상의 보고서명/논문 제목 날조 절대 금지 (매우 중요)]**:
     * 존재하지 않는 가상의 보고서명이나 논문 제목, 가공의 연구 출처를 따옴표('')로 묶어 지어내지 마십시오. (예: 존재하지 않는 '로컬 브랜드 성공 전략 연구' 등 임의 작명 금지)
     * 특정 출처가 불분명할 경우 가상의 제목을 날조하지 말고, "관련 실태조사에 따르면", "한국은행 및 주요 금융기관 발표에 따르면"과 같이 사실과 맥락 중심의 객관적 표현으로 서술하십시오.
   - **[과거 폐지된 명칭 사용 금지 및 최신 공식 기관명 사용 필수]**:
     * 과거에 폐지되었거나 변경된 명칭(예: '중소기업청' -> 2017년에 승격 폐지됨)을 절대 사용하지 마십시오.
     * 반드시 '중소벤처기업부', '중소벤처기업연구원', '소상공인시장진흥공단', '중소벤처기업진흥공단', '서울신용보증재단' 등 현재 공식 정부 부처 및 공공기관 명칭만 정확히 사용하십시오.
...
2. [소기업·소상공인 3대 핵심 사안 선정 및 심층 분석 (news_insights)]:
   ...
   - **summary (주요 팩트 및 배경)**:
     - 단순 1~2줄 요약이 아니라, 관련 보도들의 핵심 팩트, 구체적인 통계 수치, 발생 배경 및 업계 파급 효과를 5~7문장(약 300~400자)으로 깊이 있게 종합 서술하십시오.
...
3. [문체 및 절대 금지 규칙]:
   - **[가상의 보고서명/논문 제목 날조 절대 금지 (매우 중요)]**:
     * 존재하지 않는 가상의 보고서명이나 논문 제목, 가공의 연구 출처를 따옴표('')로 묶어 지어내지 마십시오. (예: 존재하지 않는 '로컬 브랜드 성공 전략 연구' 등 임의 작명 금지)
     * 특정 출처가 불분명할 경우 가상의 제목을 날조하지 말고, "관련 실태조사에 따르면", "공식 통계에 따르면"과 같이 사실과 맥락 중심의 객관적 표현으로 서술하십시오.
   - **[과거 폐지된 명칭 사용 금지 및 최신 공식 기관명 사용 필수]**:
     * 과거에 폐지되었거나 변경된 명칭(예: '중소기업청' -> 2017년에 승격 폐지됨)을 절대 사용하지 마십시오.
     * 반드시 '중소벤처기업부', '중소벤처기업연구원', '소상공인시장진흥공단', '중소벤처기업진흥공단', '서울신용보증재단' 등 현재 공식 정부 부처 및 공공기관 명칭만 정확히 사용하십시오.
...
(주의: JSON 문자열 내부에 큰따옴표(")를 사용할 경우 반드시 백슬래시(\\)로 이스케이프 처리하세요.)
{{
  "economic_trend": "주간 거시 경제 흐름 종합 진단 내용 (400~600자, 문단 간 \\n\\n 포함)",
  "news_insights": [
    {{
      "issue_title": "핵심 사안 제목 (1줄 헤드라인)",
      "summary": "주요 시장 팩트 및 배경 분석 (5~7문장, 약 300~400자)",
      "implication": "소상공인 실전 대응 전략 및 다채로운 벤치마킹/실행 가이드 (2~3개 문단, 약 400~600자, 문단 간 \\n\\n 포함)",
      "references": [
