...
오늘의 주요 뉴스들을 분석하여 재단 업무(소상공인 지원, 신용보증, 컨설팅 등)에 실질적 도움이 될 인사이트 리포트를 작성해 주세요.

[분석 대상 뉴스 목록]
{chr(10).join(news_texts)}

[작성 가이드라인]
1. **전문성 및 격식**: 공공기관 내부 보고서 수준의 정중하고 명확한 문체를 사용하십시오. 이모지는 절대 금지합니다.
2. **구조**:
   - [주요 경제 흐름]: 전체 뉴스에서 도출되는 핵심적인 거시 경제 상황을 정리. 내용이 길어지면 가독성을 위해 문단을 나누어 작성.
   - [재단 업무 인사이트]: 가장 중요한 뉴스 3~5개를 선정하여 아래 형식으로 기술.
     * 뉴스 제목 (예: 1. 제목, 2. 제목 순으로 번호 부여)
     * 주요내용 요약: 해당 뉴스의 핵심 사실 관계 요약.
     * 업무 시사점: 재단 업무(보증 심사, 소상공인 컨설팅, 상권 활성화 지원, 정책 제언 등)와의 구체적 연계성 및 대응 방안.

3. **주의사항**:
   - 마크다운 기호(#, ##, ### 등)를 절대 사용하지 마십시오. 소제목은 대괄호([])를 사용하여 표시하십시오.
   - [향후 대응 방향] 섹션은 작성하지 마십시오.
   - 각 항목은 줄바꿈을 통해 명확히 구분하십시오.
