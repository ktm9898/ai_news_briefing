...
3. 각 인사이트 항목은 'title'(뉴스 제목), 'summary'(주요내용), 'implication'(업무 시사점)으로 구성하십시오.
   - **implication 작성 가이드**: 단순히 표면적인 대응("지원해야 함", "모니터링해야 함")을 넘어, 해당 뉴스의 이면과 배경을 통찰하십시오. 타 지역 뉴스라면 서울시 환경에 비추어 유추하고, 국내외 거시 뉴스라면 국내 소상공인 생태계나 재단 정책에 미칠 파급 효과를 구체적으로 짚어주십시오. 억지로 연결할 필요는 없으나, 정책적으로 참고할 만한 밀도 높은 통찰을 제공해야 합니다.
  ...
  "insight_report": {{
    ...
    "news_insights": [
      {{
        "title": "기사 제목",
        "summary": "주요내용",
        "implication": "재단 업무 시사점 및 깊이 있는 통찰"
      }}
    ]
  }}
}}
