...
제공된 지난 일주일간({date_range_str})의 뉴스 목록 전체를 심층 분석하여, 바쁜 서울시 소기업 및 소상공인 사장님들의 실질적 사업 운영과 의사결정에 직결되는 주간 인사이트 리포트를 작성해 주세요.
...
2. [소기업·소상공인 3대 핵심 사안 선정 및 심층 분석 (news_insights)]:
   ...
   - **implication (인사이트 및 실전 대응 전략)**:
     ...
     - **[문체 및 서술 가이드 - 상투적 패턴 반복 금지]**:
       * '단기적으로는~, 중장기적으로는~' 같은 도식적이고 기계적인 문구 틀을 세 이슈 모두에 일률적으로 반복하지 마십시오.
       * 각 사안의 고유한 성격에 맞춰 자연스럽고 다채로운 호흡으로 서술하십시오:
         - 금융/원가 압박 사안: 구체적 재무 방어 및 채무 조정 요령, 지자체·재단 정책금융 활용법, 원가 구조 혁신 등을 유기적으로 연결
         - 소비 트렌드/상권 변화 사안: 타깃 고객 공략법, 매장 공간/메뉴/서비스 차별화, 로컬 브랜딩 및 디지털 접점 확대 방안 제시
         - 제도/환경 변화 사안: 신규 제도 대응 체크리스트, 사업 모델 전환 및 정부 지원사업 연계 방안 제시
       * 사안의 성격에 적합한 국내외 실제 성공/극복 벤치마킹 사례(검증된 실제 팩트 기반)를 자연스럽게 녹여내십시오.
...
(주의: JSON 문자열 내부에 큰따옴표(")를 사용할 경우 반드시 백슬래시(\\)로 이스케이프 처리하세요.)
{{
  "economic_trend": "주간 거시 경제 흐름 종합 진단 내용 (800~1000자, 문단 간 \\n\\n 포함)",
  "news_insights": [
    {{
      "issue_title": "핵심 사안 제목 (1줄 헤드라인)",
      "summary": "주요 시장 팩트 및 배경 분석 (5~7문장, 약 400~500자)",
      "implication": "소상공인 실전 대응 전략 및 벤치마킹 사례 (2~3개 문단, 약 600~800자, 문단 간 \\n\\n 포함)",
      "references": [
