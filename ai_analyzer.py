...
- **[매우 중요 규칙 - 저작권 및 템플릿 준수]**:
  1. 특정 언론사의 기사 제목이나 언론사명을 직접 사용하지 마십시오.
  2. 기사들의 핵심 팩트를 통합하여 **"ISSUE 1, 2, 3"에 들어갈 1줄 이슈 헤드라인 제목 (`issue_title`)**을 직접 새로 다십시오. (예: "고금리 장기화에 따른 소상공인 대출 연체 및 경영 난항")
  3. **요약(`summary`)**은 특정 기사를 복제하지 말고 팩트(Fact) 위주로 3~4문장으로 새로 구성하십시오.
  4. **인사이트(`implication`)**는 소상공인을 위한 재정 지원, 경영 안정, 디지털 전환 등 구체적인 대응 방안을 제언하십시오.
  5. 반드시 총 3개의 주요 이슈 항목을 도출하십시오.
...
{{
  ...
  "news_insights": [
    {{
      "issue_title": "주요 이슈 제목 (1줄 헤드라인)",
      "summary": "주요내용 요약 (팩트 위주)",
      "implication": "소상공인 사업운영을 위한 구체적인 인사이트 및 대응 방향",
