...
3. 각 인사이트 항목은 'title'(뉴스 제목), 'summary'(주요내용), 'implication'(시사점)으로 구성하십시오.
   - **implication 작성 가이드**: 단순히 표면적인 대응("지원해야 함", "모니터링해야 함")을 넘어, 해당 뉴스의 이면과 배경을 통찰하십시오. 타 지역 뉴스라면 서울시 환경에 비추어 유추하고, 국내외 거시 뉴스라면 국내 소상공인 생태계나 재단 정책에 미칠 파급 효과를 구체적으로 짚어주십시오. 특히, 실질적인 도움이 될 수 있도록 **관련된 국내외 유사 사례, 과거 벤치마킹 사례 또는 성공/실패 레퍼런스를 1~2개 이상 구체적으로 포함**하여 밀도 높은 통찰을 제공해야 합니다. (AI의 방대한 지식망을 적극 활용하십시오.)
